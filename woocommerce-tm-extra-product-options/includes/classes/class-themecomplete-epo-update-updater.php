...
final class THEMECOMPLETE_EPO_UPDATE_Updater {
	...
	public function upgrade_filter_from_envato( $reply, $package, $updater ) {
		...
		if ( ( isset( $updater->skin->plugin ) && THEMECOMPLETE_EPO_PLUGIN_SLUG === $updater->skin->plugin ) ||
			...
			if ( $wordpress_plugin && property_exists( $result, 'wordpress_plugin' ) ) {
				$download_file = download_url( $result->wordpress_plugin );
			} elseif ( property_exists( $result, 'download_url' ) ) {
...
final class THEMECOMPLETE_EPO_UPDATE_Manager {
	 ...
	 * @var string
	 */
	protected $url = 'http://themecomplete.com/downloadnew';
	...
	public function __construct( $current_version, $update_path, $plugin_slug, $instance ) {
		...
		add_action( 'in_plugin_update_message-' . $this->plugin_slug, [ $this, 'tm_update_message' ], 10, 1 );
	}
	 ...
	 */
	public function tm_update_plugins( $transient = null ) {
		if ( $transient && is_object( $transient ) && property_exists( $transient, 'response' ) ) {
	 ...
	 * @since 1.0
	 */
	public function tm_update_message( $plugin_data = [] ) {
		$plugins = get_plugins();
		if ( ! THEMECOMPLETE_EPO_LICENSE()->check_license() ) {
			echo '<br /><span class="tc-update-register-message"><a href="' . esc_url( $this->url ) . '">' . esc_html__( 'Download new version from CodeCanyon', 'woocommerce-tm-extra-product-options' ) . '</a> ' . esc_html__( 'or register the plugin to receive automatic updates.', 'woocommerce-tm-extra-product-options' ) . '</span>';
		}

		if ( is_array( $plugin_data ) && isset( $plugin_data['new_version'] ) && version_compare( THEMECOMPLETE_EPO_VERSION, $plugin_data['new_version'], '<' ) ) {
			echo '<br /><span class="tc-update-message">' . esc_html__( 'Please make sure the cache is cleared after each plugin update especially if you have minified JS and/or CSS files.', 'woocommerce-tm-extra-product-options' ) . '</span>';
		}
	}
}
