...
final class THEMECOMPLETE_Extra_Product_Options {
	...
	/**
	 * Cache for loading scripts
	 *
	 * @var array<mixed>
	 */
	public $can_load_scripts = [];
	...
	public function __construct() {
		...
		add_action( 'init', [ $this, 'tm_epo_add_elements' ], 9 );
	}
	 ...
	 * @since 1.0
	 */
	public function is_visible( $element = [], $section = [], $sections = [], $form_prefix = '', $product_id = 0, $variation_id = 0, $qty = 0 ) {
		return THEMECOMPLETE_EPO_CONDITIONAL_LOGIC()->is_visible( $element, $section, $sections, $form_prefix, $product_id, $variation_id, $qty );
	}
	...
	public function add_plugin_actions() {
		...
		add_filter( 'woocommerce_cart_shipping_packages', [ $this, 'woocommerce_cart_shipping_packages' ], 10000, 1 );
		...
		if ( 'yes' === THEMECOMPLETE_EPO_DATA_STORE()->get( 'tm_epo_global_quantity_selector_buttons' ) ) {
			add_action( 'wc_epo_quantity_selector_before_input', [ $this, 'wc_epo_quantity_selector_before_input' ], 10, 4 );
			add_action( 'wc_epo_quantity_selector_after_input', [ $this, 'wc_epo_quantity_selector_after_input' ], 10, 4 );
		}
		if ( 'yes' === THEMECOMPLETE_EPO_DATA_STORE()->get( 'tm_epo_global_quantity_buttons_product_element' ) ) {
			add_action( 'wc_epo_before_product_quantity_input_field', [ $this, 'wc_epo_quantity_selector_before_input' ], 10, 4 );
			add_action( 'wc_epo_after_product_quantity_input_field', [ $this, 'wc_epo_quantity_selector_after_input' ], 10, 4 );
		}

		// Show Prices on Styled Variation Attributes.
		add_filter( 'wc_epo_variations_display_field', [ $this, 'wc_epo_variations_display_field' ], 10, 4 );
	}

	/**
	 * Modifies the display of a variation attribute field to include the price of the matching variation.
	 *
	 * This function checks if a product variation exists for the given attribute name and value.
	 * If found, it retrieves the price of that variation and appends it to the display label.
	 *
	 * Only works when each variation is defined by a single attribute and the matching product variation can be found.
	 *
	 * @param array      $display    The display array containing the original label to show.
	 * @param string     $name       The attribute name (e.g., 'pa_color').
	 * @param string     $value      The selected attribute value (e.g., 'red').
	 * @param WC_Product $tm_product The main WooCommerce product object used to search for matching variations.
	 *
	 * @return array Modified display array with the variation price appended to the label, if a match is found.
	 */
	public function wc_epo_variations_display_field( $display, $name, $value, $tm_product ) {
		if ( 'yes' === THEMECOMPLETE_EPO_DATA_STORE()->get( 'tm_epo_styled_variation_price_display' ) && class_exists( 'WC_Product_Data_Store_CPT' ) ) {
			$attribute_slug               = 'attribute_' . sanitize_title( $name );
			$_attributes                  = [
				$attribute_slug => (string) $value,
			];
			$w_c_product_data_store_c_p_t = new WC_Product_Data_Store_CPT();
			$variation_id                 = $w_c_product_data_store_c_p_t->find_matching_product_variation( $tm_product, $_attributes );
			if ( $variation_id ) {
				$_variation = wc_get_product( $variation_id );
				$price      = $_variation->get_price();

				$label_to_display = '<div class="tc-price-variations">';
				if ( '' !== $display['label_to_display'] ) {
					$label_to_display .= '<div class="tc-price-variation-label">'
					. $display['label_to_display']
					. '</div>';
				}
				$label_to_display .= '<div class="tc-price-variation-price">'
				. wc_price( $price )
				. '</div>'
				. '</div>';

				$display['label_to_display'] = $label_to_display;
			}
		}
		return $display;
	}
	...
	/**
	 * Outputs a minus button before the quantity input field.
	 *
	 * @param string $operation_mode The operation mode of the quantity selector.
	 * @param int    $input_value    The current input value.
	 * @param int    $min_value      The minimum allowable value.
	 * @param int    $max_value      The maximum allowable value.
	 * @since 7.1
	 */
	public function wc_epo_quantity_selector_before_input( $operation_mode, $input_value = 0, $min_value = 0, $max_value = 0 ) {
		if ( $max_value && $min_value === $max_value && $input_value === $max_value ) {
			return;
		}
		echo '<button type="button" class="minus button tc-qty-button">-</button>';
	}

	/**
	 * Outputs a plus button after the quantity input field.
	 *
	 * @param string $operation_mode The operation mode of the quantity selector.
	 * @param int    $input_value    The current input value.
	 * @param int    $min_value      The minimum allowable value.
	 ...
	 * @since 7.1
	 */
	public function wc_epo_quantity_selector_after_input( $operation_mode, $input_value = 0, $min_value = 0, $max_value = 0 ) {
		if ( $max_value && $min_value === $max_value && $input_value === $max_value ) {
			return;
		}
		echo '<button type="button" class="plus button tc-qty-button">+</button>';
	}
	 ...
	 * Add the extra weight from addons to the product.
	 *
	 * We use this filter since shipping is done before calcualting totals on checkout.
	 *
	 * @param array $packages An array of shipping packages, where each package
	 *                        contains cart contents, applied coupons, destination, etc.
	 * @return array The modified shipping packages.
	 */
	public function woocommerce_cart_shipping_packages( $packages = [] ) {
		$this->add_custom_weight();
		return $packages;
	}
			...
			switch ( $tm_price_display_mode ) {
				case 'price':
					if ( '' !== $tm_price_display_override_sale ) {
						...
						$tax_price = $tm_price_display_override_sale;
					} else {
						$price     = themecomplete_price( wc_format_decimal( $f_tm_price_display_override ) );
						$tax_price = $f_tm_price_display_override;
					}
				...
				case 'range':
					...
					$tax_price = [ $f_tm_price_display_override, $f_tm_price_display_override_to ];
					break;
			}

			$taxable    = $product->is_taxable();
			$tax_string = '';
			if ( $taxable && method_exists( $product, 'get_price_suffix' ) ) {
				if ( '' !== $tax_price ) {
					if ( is_array( $tax_price ) ) {
						foreach ( $tax_price as $k => $tprice ) {
							$tax_price[ $k ] = $product->get_price_suffix( $tprice );
						}
						$tax_price = join( ' - ', $tax_price );
					} else {
						$tax_price = $product->get_price_suffix( $tax_price );
					}
				}
				$tax_string .= ' <small>' . $tax_price . '</small>';
	...
	public function init_settings_after() {
		...
		do_action( 'wc_epo_init_settings_after_start' );
		...
		do_action( 'wc_epo_init_settings_after_end' );
	}
	...
	public function hasoptions() {
		$post_id = get_queried_object_id(); // Works for any post type.
		if ( $post_id && ! isset( $this->hasoptions[ $post_id ] ) ) {
			if ( $this->wc_vars['is_product'] || 'product' === get_post_type( $post_id ) ) {
	...
	public function can_load_scripts() {
		$current_id = get_queried_object_id(); // Works for any post type.
		if ( isset( $this->can_load_scripts[ $current_id ] ) ) {
			return $this->can_load_scripts[ $current_id ];
		}
		...
		foreach ( $loading_areas as $area ) {
			switch ( $area ) {
				...
				case 'quickview':
					...
					break;
				case 'homepage':
					$can_load_scripts = $can_load_scripts || is_front_page();
					break;
				case 'pages':
					$can_load_scripts = $can_load_scripts || ( is_page() && ! is_woocommerce() );
					break;
				case 'specific_ids':
					$allowed_ids = THEMECOMPLETE_EPO_DATA_STORE()->get( 'tm_epo_loading_areas_allowed_ids' );
					if ( ! empty( $allowed_ids ) ) {
						$allowed_ids = array_map( 'intval', explode( ',', $allowed_ids ) );
						$current_id  = get_queried_object_id(); // Works for any post type.
						if ( $current_id ) {
							$can_load_scripts = $can_load_scripts || in_array( $current_id, $allowed_ids, true );
						}
					}
					break;
		...
		if ( $current_id && apply_filters( 'wc_epo_disable_scripts_if_no_extra_options', true ) && $can_load_scripts && THEMECOMPLETE_EPO()->wc_vars['is_product'] ) {
			if ( ! isset( $this->hasoptions[ $current_id ] ) ) {
				$this->hasoptions();
			}
			if ( ! $this->hasoptions[ $current_id ] ) {
				$this->can_load_scripts[ $current_id ] = false;
				return false;
			}
		}
		...
		$this->can_load_scripts[ $current_id ] = $can_load_scripts;
	...
	public function calculate_math_price( $_price = '', $post_data = [], $element = [], $key = '', $attribute = false, $attribute_quantity = false, $key_id = 0, $keyvalue_id = 0, $per_product_pricing = null, $cpf_product_price = false, $variation_id = 0, $price_default_value = 0, $currency = false, $current_currency = false, $price_per_currencies = null, $tmcp = false, $cart_meta_epo_type = false, $tmdata = [], $func_total = false, $cumulative_total = false ) {
		$formula = apply_filters( 'wc_epo_replace_math_currency', $_price );
		...
		if ( isset( $element['options'] ) && isset( $element['options'][ $key ] ) ) {
			...
			$formula = str_replace( '{this.value}', THEMECOMPLETE_EPO_HELPER()->convert_to_number( floatval( THEMECOMPLETE_EPO_HELPER()->unformat( $element['options'][ $key ] ) ), true ), $formula );
			...
			// the option/element value to float.
			$formula = str_replace( '{this.value}', THEMECOMPLETE_EPO_HELPER()->convert_to_number( floatval( THEMECOMPLETE_EPO_HELPER()->unformat( $attribute_value ) ), true ), $formula );
			// the option/element value.
		...
		if ( $per_product_pricing && '' !== $key ) {
			...
			switch ( $_price_type ) {
				...
				case 'charnonnospaces':
					$freechars   = absint( $element['freechars'] );
					$_textlength = floatval( tc_strlen( preg_replace( '/\s+/u', '', $posted_attribute ), true ) ) - $freechars;
					if ( $_textlength < 0 ) {
						$_textlength = 0;
					}
					$_price = floatval( floatval( $_price ) * $_textlength );
				...
				case 'charnospaces':
					$_price = floatval( floatval( $_price ) * tc_strlen( preg_replace( '/\s+/u', '', $posted_attribute ), true ) );
					if ( $currency ) {
						$_price = apply_filters( 'wc_epo_get_currency_price', $_price, $currency, $_price_type, $current_currency, $price_per_currencies, $key, $attribute );
					}
					break;
	...
	public function fetch_all_lookuptables() {
		...
		do_action( 'wc_epo_before_fetch_all_lookuptables' );
		...
		$args = apply_filters(
			'wc_epo_fetch_all_lookuptables_query_args',
			[
		...
		do_action( 'wc_epo_after_fetch_all_lookuptables', $lookuptables );

		return apply_filters( 'wc_epo_fetch_all_lookuptables', $lookuptables );
	}
	...
	public function upload_file( $file, $key_id = 0, $keyvalue_id = 0 ) {
		...
		if ( is_array( $tmp_name ) ) {
			while ( is_array( $tmp_name ) && ! empty( $tmp_name ) ) {
				$last_key = array_key_last( $tmp_name );
				$tmp_name = $tmp_name[ $last_key ];
			}
		}
		...
		if ( ! $this->tm_meta_cpf['exclude'] ) {
			...
			if ( $tmglobalprices_products ) {
				...
				foreach ( $tmglobalprices_products as $price ) {
					$tm_meta_product_ids = themecomplete_get_post_meta( $price->ID, 'tm_meta_product_ids', true );
					if ( ! is_array( $tm_meta_product_ids ) ) {
						continue;
					}
					$tm_meta_product_ids = array_map( 'absint', $tm_meta_product_ids );

					if ( ! in_array( $original_post_id, $tm_meta_product_ids, true ) ) {
						if ( is_array( $available_variations ) && ! empty( $available_variations ) ) {
							foreach ( $available_variations as $variation_id ) {
								if ( in_array( $variation_id, $tm_meta_product_ids, true ) ) {
									break;
								}
								continue;
							}
			...
			if ( floatval( $post_id ) !== $post_original_id ) {
				...
				if ( $tmglobalprices_products ) {
					...
					foreach ( $tmglobalprices_products as $price ) {
						...
						if ( ! in_array( $post_original_id, $tm_meta_product_ids, true ) ) {
							continue;
						}
	...
	public function generate_local_epos( $tmlocalprices, $post_id ) {
		...
		if ( $tmlocalprices ) {
			THEMECOMPLETE_EPO_WPML()->remove_sql_filter();
			$attributes      = themecomplete_get_attributes( absint( THEMECOMPLETE_EPO_WPML()->get_original_id( $post_id ) ), true );
			$wpml_attributes = themecomplete_get_attributes( $post_id, true );
						...
						if ( $all_terms ) {
							foreach ( $all_terms as $term ) {
								if ( $term instanceof WP_Term ) {
									...
									$wpml_term_id = THEMECOMPLETE_EPO_WPML()->is_active() && function_exists( 'wpml_object_id_filter' ) ? wpml_object_id_filter( $term->term_id, $attributes[ $product_epos[ $tmcp_id ]['name'] ]['name'], false ) : false;
									if ( $has_term ) {
	...
	/**
	 * Generates a header element structure.
	 *
	 * This function returns an associative array representing a header element,
	 * including global settings, pricing information, variation settings, and product choices.
	 *
	 * @return array The structured header element data.
	 */
	private function header_element_data() {
		$section_uniqid = '0';
		$uniqid         = '1';
		return [
			'global'               => [
					...
					14392 => [
						...
						'sections'    => [
							0 => [
								...
								'sections_uniqid'          => $section_uniqid,
								'sections_logicrules'      => '',
								'sections_logic'           => '',
								'sections_class'           => '',
								'sections_type'            => '',
								'sections_popupbutton'     => '',
			...
			'price'                => [
				0 => [
					...
					'required'           => '',
					'element'            => 0,
					'section_uniqueid'   => $section_uniqid,
					'minall'             => 0.0,
					'min'                => 0.0,
					'max'                => 0.0,
					'logicrules'         => '',
								...
								if ( $element_object ) {
									...
									if ( $is_override_element ) {
										if ( THEMECOMPLETE_EPO_ADMIN()->is_backend_enabled() ) {
											if ( $this->is_bto ) {
												$builder[ $current_element . '_header_title' ][ $current_counter ]         = esc_html__( 'Product element is not supported for components!', 'woocommerce-tm-extra-product-options' );
												$current_builder[ $current_element . '_header_title' ][ $current_counter ] = esc_html__( 'Product element is not supported for components!', 'woocommerce-tm-extra-product-options' );
											}
											if ( $this->is_associated ) {
												$builder[ $current_element . '_header_title' ][ $current_counter ] = esc_html__( 'Product element is not supported within another product element!', 'woocommerce-tm-extra-product-options' );
											}
										}
									}
									...
									if ( '' !== $logicrules ) {
										$logicrules = THEMECOMPLETE_EPO_CONDITIONAL_LOGIC()->convert_rules( json_decode( wp_unslash( $logicrules ) ) );
										$logicrules = THEMECOMPLETE_EPO_CONDITIONAL_LOGIC()->convert_rules( $logicrules );
										$logicrules = wp_json_encode( $logicrules );
									}
									...
									if ( $is_enabled && 'template' === $current_element ) {
										...
										if ( ! empty( $templateids ) ) {
											$templateids          = THEMECOMPLETE_EPO_WPML()->get_current_lang_id( $templateids );
										...
										if ( empty( $template_elements ) ) {
											$template_elements = $this->header_element_data();
										}
									...
									if ( 'single' === $element_object->type || 'multipleallsingle' === $element_object->type ) {
										...
										} elseif ( 'math' === $this_price_type ) {
											// Replace currency number placeholders with the number.
											$_price = preg_replace_callback(
												'/\{(\d+)\}/u',
												function ( $matches ) {
													return $matches[1];
												},
										...
										if ( isset( $builder[ 'multiple_' . $current_element . '_options_price' ][ $current_counter ] ) ) {
											...
											$_dnmpbq      = $this->get_builder_element( 'multiple_' . $current_element . '_options_dnmpbq', $builder, $current_builder, $current_counter, [], $current_element, '', $element_uniqueid );
											...
											foreach ( $_prices as $_n => $_price ) {
												...
												if ( isset( $_enabled[ $_n ] ) && ( '0' === $_enabled[ $_n ] || '' === $_enabled[ $_n ] ) ) {
													...
													if ( isset( $_dnmpbq ) && is_array( $_dnmpbq ) ) {
														unset( $_dnmpbq[ $_n ] );
													}
										...
										$this->cpf_single[ $element_uniqueid ] =
											array_merge(
												...
												[
													...
													'hide_element_label_in_cart' => $this->get_builder_element( $_prefix . 'hide_element_label_in_cart', $builder, $current_builder, $current_counter, false, $current_element, '', $element_uniqueid ),
													'hide_element_value_in_cart' => $this->get_builder_element( $_prefix . 'hide_element_value_in_cart', $builder, $current_builder, $current_counter, false, $current_element, '', $element_uniqueid ),
													'hide_element_label_in_order' => $this->get_builder_element( $_prefix . 'hide_element_label_in_order', $builder, $current_builder, $current_counter, false, $current_element, '', $element_uniqueid ),
													'hide_element_value_in_order' => $this->get_builder_element( $_prefix . 'hide_element_value_in_order', $builder, $current_builder, $current_counter, false, $current_element, '', $element_uniqueid ),
													'hide_element_label_in_floatbox' => $this->get_builder_element( $_prefix . 'hide_element_label_in_floatbox', $builder, $current_builder, $current_counter, false, $current_element, '', $element_uniqueid ),
													'hide_element_value_in_floatbox' => $this->get_builder_element( $_prefix . 'hide_element_value_in_floatbox', $builder, $current_builder, $current_counter, false, $current_element, '', $element_uniqueid ),
													...
													'dnmpbq' => $this->get_builder_element( $_prefix . 'dnmpbq', $builder, $current_builder, $current_counter, '', $current_element, '', $element_uniqueid ),
													...
													'is_dnmpbq_multiple' => isset( $_dnmpbq ) ? $_dnmpbq : [],
	...
	public function get_post_names( $attributes, $type, $field_loop = '', $form_prefix = '', $name_prefix = '', $element = [] ) {
		...
		if ( ! empty( $attributes ) ) {
			...
			if ( ! empty( $element_object->type ) && ! empty( $element_object->post_name_prefix ) ) {
				$name_inc = 'tmcp_' . $name_prefix . $element_object->post_name_prefix . '_' . $field_loop . $form_prefix;
				if ( is_array( $element ) && isset( $element['mode'] ) && 'product' !== $element['mode'] && isset( $element['type'] ) && 'product' === $element['type'] && isset( $element['layout_mode'] ) && ( 'checkbox' === $element['layout_mode'] || 'thumbnailmultiple' === $element['layout_mode'] || 'custombundle' === $element['layout_mode'] ) ) {
					$name_inc = $name_inc . '_*';
				}
			}
	...
	public function tm_fill_element_names( $post_id = 0, $global_epos = [], $product_epos = [], $form_prefix = '', $add_identifier = '' ) {
		...
		if ( is_array( $local_price_array ) && count( $local_price_array ) > 0 ) {
			$attributes = themecomplete_get_attributes( $post_id, true );
