...
<tr class="tm-order-line-option item <?php echo esc_attr( (string) $row_class ); ?>" data-order_item_id="<?php echo esc_attr( (string) $item_id ); ?>" data-tm_item_id="<?php echo esc_attr( (string) $item_id ); ?>" data-tm_key_id="<?php echo esc_attr( (string) $key ); ?>">
	...
	<td class="item_cost" width="1%" data-sort-value="<?php echo esc_attr( $order->get_item_subtotal( $item, false, true ) ); ?>">
		...
		if ( $epo['quantity'] <= 0 ) {
			...
			echo '</div>';
			if ( $epo_can_show_order_price && $epo_edit_cost ) {
				echo '<div class="edit tc"><input novalidate type="' . esc_attr( (string) $input_type ) . '" name="tm_epo[' . esc_attr( (string) $item_id ) . '][' . esc_attr( (string) $key ) . '][price]" placeholder="0" value="';
				echo esc_attr(
			...
			if ( $epo_can_show_order_price ) {
				...
				if ( $epo_is_fee ) {
					...
					echo wc_price(
						...
						) * ( empty( $is_dnmpbq ) ? (float) $item_meta['_qty'][0] : 1 ),
						$currency_arg
					);
				}
				echo '</span>';
	...
	if ( ! empty( $tax_data ) ) {
		...
		if ( isset( $epo['tax_class_for_fee_price_type'] ) && '' !== $epo['tax_class_for_fee_price_type'] && '@' !== $epo['tax_class_for_fee_price_type'] ) {
			$tax_class = $epo['tax_class_for_fee_price_type'];
		}
