<?php
/**
 * Extra Product Options Admin Builder class
 *
 ...
 * @version 6.4
 */
final class THEMECOMPLETE_EPO_ADMIN_BUILDER_Base {

	/**
	 * The single instance of the class
	 *
	 * @var THEMECOMPLETE_EPO_ADMIN_BUILDER_Base|null
	 * @since 6.4
	 */
	...
	/**
	 * Extracts a numeric value from a class name string.
	 *
	 * This method processes class names like `w50`, `w12-5`, etc. by removing the
	 * `w` prefix and replacing any dash (`-`) with a dot (`.`) for handling decimal values.
	 * It returns the extracted number as a float. If no valid number is found, it defaults to `100`.
	 *
	 * @param string $class_name The class name containing the numeric value to extract (e.g., 'w50', 'w12-5').
	 *
	 * @return float The numeric value extracted from the class name. Returns `100` if no valid number is found.
	 */
	private function extract_number( $class_name = '' ) {
		// Remove the 'w' prefix (since it's always the same).
		$class_name = str_replace( 'w', '', $class_name );

		// Replace a dash (-) with a dot (.) to handle cases like "w12-5".
		$class_name = str_replace( '-', '.', $class_name );

		// Return as a float or 100 if no valid number is found.
		return '' !== $class_name ? (float) $class_name : 100;
	}
	...
	private function init_elements() {
		$all_elements = THEMECOMPLETE_EPO_BUILDER()->get_elements();
		do_action( 'wc_epo_builder_before_element_set_properties', $all_elements );

		foreach ( $all_elements as $element => $class ) {
			$class->set_properties();
		}

		do_action( 'wc_epo_builder_after_element_set_properties', $all_elements );
	}
	...
	public function add_setting_rangemin( $name = '' ) {
		return $this->add_setting_min( $name, [ 'default' => '1' ], false );
	}
	...
	public function add_setting_rangemax( $name = '' ) {
		return $this->add_setting_max( $name, [ 'default' => '100' ], false );
	}
	 ...
	 * @since 1.0
	 */
	public function add_setting_rangestep( $name = '', $args = [] ) {
		return array_merge(
			[
				'id'          => $name . '_step',
				'wpmldisable' => 1,
	 ...
	 * @since 1.0
	 */
	public function add_setting_price( $name = '', $args = [] ) {
		return array_merge(
			[
				'id'       => $name . '_price',
				'default'  => '',
	 ...
	 * @since 1.0
	 */
	public function add_setting_text_after_price( $name = '', $args = [] ) {
		return array_merge(
			[
	 ...
	 * @since 1.0
	 */
	public function add_setting_text_before_price( $name = '', $args = [] ) {
		return array_merge(
			[
	...
	/**
	 * Dynamic Calculations price type setting
	 *
	 * @param string $name Element name.
	 * @return array<mixed>
	 * @since 6.4.3
	 */
	public function add_setting_price_type_dynamic( $name = '' ) {
		...
		return [
			...
			'options'     => $options,
			'label'       => esc_html__( 'Calculation type', 'woocommerce-tm-extra-product-options' ),
		];
	}
	...
	/**
	 * Do Not Multiply Price by Quantity setting
	 *
	 * @param string       $name Element name.
	 * @param array<mixed> $args Array of arguments.
	 * @return array<mixed>
	 * @since 5.0
	 */
	public function add_setting_dnmpbq( $name = '', $args = [] ) {
		$setting = array_merge(
			[
				'id'          => $name . '_dnmpbq',
				'wpmldisable' => 1,
				...
				'type'        => 'checkbox',
				'tags'        => [
					'class' => 'c tc-element-setting-dnmpbq',
					'id'    => 'builder_' . $name . '_dnmpbq',
					'name'  => 'tm_meta[tmfbuilder][' . $name . '_dnmpbq][]',
					'value' => '1',
				],
				'label'       => esc_html__( 'Do Not Multiply Price by Quantity', 'woocommerce-tm-extra-product-options' ),
				'desc'        => esc_html__( 'Applies the addon price once, regardless of how many units of the product are ordered.', 'woocommerce-tm-extra-product-options' ),
				'required'    => [
		...
		$setting = apply_filters( 'wc_epo_add_setting_dnmpbq', $setting, $name, $args );
	 ...
	 * @param string       $name Element name.
	 * @param array<mixed> $args Array of arguments.
	 * @param array<mixed> $required If the setting is required.
	 * @return array<mixed>
	 * @since 1.0
	 */
	public function add_setting_items_per_row( $name = '', $args = [], $required = [] ) {
			...
			'_multiple_values' => [
				[
					...
					'tags'        => [
						...
						'placeholder' => esc_html__( 'auto', 'woocommerce-tm-extra-product-options' ),
					],
	...
	public function template_bitem( $args = [] ) {
		...
		if ( empty( $this->noecho ) ) {
			echo '<div data-size="' . esc_attr( $this->extract_number( $args['width'] ) ) . '" class="bitem element-' . esc_attr( $args['element'] ) . esc_attr( $is_enabled ) . ' ' . esc_attr( $args['width'] ) . '">';
			...
			if ( $show_buttons ) {
				if ( $wpml_is_original_product ) {
					...
					echo '</li>';
					echo '<li class="bitem-setting change" title="' . esc_attr__( 'Change Element Type', 'woocommerce-tm-extra-product-options' ) . '">';
					echo '<i class="tmicon tcfa tcfa-exchange-alt"></i>';
					echo '</li>';
				}
			}
			echo '</ul>';
			...
			if ( $show_buttons && $wpml_is_original_product ) {
			...
			if ( empty( $args['is_template'] ) ) {
				...
				if ( $pointer >= 0 ) {
					...
					foreach ( $args['fields'] as $value ) {
						if ( ! empty( $value['clogic'] ) ) {
							$clogic = $value;
						}
						if ( ! empty( $value['logicrules'] ) ) {
							$logicrules          = $value;
							$logicrules_pointers = [
								'pointer'        => $pointer,
								'fields_pointer' => $fields_pointer,
								'jpointer'       => count( $this->jsbuilder[ $pointer ]['fields'][ $fields_pointer ] ),
							];
						}
						if ( isset( $value['type'] ) && 'custom' !== $value['type'] ) {
								...
								if ( isset( $value['fill'] ) ) {
									if ( 'product' === $value['fill'] ) {
										...
										foreach ( $category_ids as $category_id ) {
											$current_category_id = absint( wc_clean( wp_unslash( $category_id ) ) );
					...
					if ( false !== $clogic && false !== $logicrules && isset( $logicrules_pointers['jpointer'] ) ) {
						if ( isset( $clogic['default'] ) && array_key_exists( 'default', $logicrules ) && null === $logicrules['default'] ) {
							...
							$this->jsbuilder[ $logicrules_pointers['pointer'] ]['fields'][ $logicrules_pointers['fields_pointer'] ][ $logicrules_pointers['jpointer'] ] = $this->remove_for_js( $logicrules );
						}
					}
				}
	...
	private function section_elements_template( $args = [] ) {
		 ...
		 * The section fields
		 *
		 * @var array<mixed>|string $section_fields
		 */
		...
		if ( empty( $this->noecho ) ) {
			...
			if ( $show_buttons ) {
				...
				echo '</li>';
				if ( $wpml_is_original_product ) {
					echo '<li class="section-setting clone" title="' . esc_attr__( 'Duplicate Section', 'woocommerce-tm-extra-product-options' ) . '">';
					echo '<i class="tmicon tcfa tcfa-copy"></i>';
					echo '</li>';
				}
				echo '</ul>';
			...
			if ( $show_buttons ) {
				...
				echo '<div class="tm-internal-label">' . esc_html( (string) $sections_internal_name ) . '</div>';
				echo '</div>';
			}
			echo '<div class="tm-for-section tm-label-desc-edit tm-hidden">';
			if ( ! empty( $args['is_template'] ) ) {
				echo '<input type="text" value="' . esc_attr( (string) $sections_internal_name ) . '" name="tm_meta[tmfbuilder][sections_internal_name][]" class="t tm-internal-name">';
			}
			echo '</div>';
			echo '</div>';
		}
	...
	private function section_template( $args = [] ) {
		...
		/**
		 * The section tab labels
		 *
		 * @var array<mixed>|string $sections_tabs_labels
		 */
		$sections_tabs_labels = $args['sections_tabs_labels'];
		if ( is_array( $sections_tabs_labels ) ) {
			$sections_tabs_labels = current( $sections_tabs_labels );
		}
		 ...
		 * The elements data
		 *
		 * @var array<mixed>|string $elements
		 */
		...
		if ( '' !== $sections_slides && is_array( $elements ) ) {
			if ( empty( $this->noecho ) ) {
				echo '<div data-size="' . esc_attr( $this->extract_number( $section_size ) ) . '" class="builder-wrapper tm-slider-wizard ' . esc_attr( $section_size ) . ' is-' . esc_attr( $sections_type ) . '"><div class="builder-section-wrap">';
			}
			...
			if ( empty( $this->noecho ) ) {
				...
				echo '<div class="transition tm-slider-wizard-headers-inner">';
				$s = 0;
				...
				foreach ( $sections_slides as $key => $value ) {
					...
					echo '<div class="tm-box"><h4 class="tm-slider-wizard-header" data-id="tc-tab-slide' . esc_attr( (string) $s ) . '"><span class="tab-text">' . esc_html( $sections_tabs_labels[ $s ] ) . '</span></h4></div>';
				...
				echo '</div>';
				echo '</div>';
				if ( $wpml_is_original_product && ( ! $post || THEMECOMPLETE_EPO_TEMPLATE_POST_TYPE !== $post->post_type ) ) {
			...
			foreach ( $sections_slides as $key => $value ) {
				...
				if ( empty( $this->noecho ) ) {
					echo "<div class='bitem-wrapper tc-tab-slide tc-tab-slide" . esc_attr( (string) $s ) . "'>";
				}
			...
			if ( empty( $this->noecho ) ) {
				echo '<div data-size="' . esc_attr( $this->extract_number( $section_size ) ) . '" class="builder-wrapper ' . esc_attr( $section_size ) . '"><div class="builder-section-wrap">';
			}
			$this->section_elements_template(
				[
					'section_fields'           => $section_fields,
	...
	public function template_elements() {
		$all_elements = THEMECOMPLETE_EPO_BUILDER()->get_elements();
		foreach ( $all_elements as $element => $settings ) {
	...
	public function print_saved_elements( $post_id = 0, $current_post_id = 0, $wpml_is_original_product = true ) {
		...
		if ( ! empty( $post_id ) && is_array( $builder ) && count( $builder ) > 0 && isset( $builder['sections'] ) && isset( $builder['element_type'] ) && is_array( $builder['element_type'] ) && count( $builder['element_type'] ) > 0 ) {
			...
			$_sections_tabs_labels = isset( $current_builder['sections_tabs_labels'] ) ? $current_builder['sections_tabs_labels'] : ( isset( $builder['sections_tabs_labels'] ) ? $builder['sections_tabs_labels'] : '' );
			...
			for ( $_s = 0; $_s < $count_sections; $_s++ ) {
				...
				foreach ( $this->section->properties as $_sk => $_sv ) {
					...
					if ( 'sections_clogic' === $_sv['id'] ) {
						...
						$_sv['default'] = wp_unslash( $_sv['default'] );
					}
					if ( 'sections_logicrules' === $_sv['id'] ) {
						if ( is_object( $_sv['default'] ) ) {
							$_sv['default'] = wp_json_encode( $_sv['default'] );
						}
						$_sv['default'] = wp_unslash( $_sv['default'] );
				...
				for ( $k0 = $_helper_counter; $k0 < (int) ( $_helper_counter + (int) $_sections[ $_s ] ); $k0++ ) {
					if ( isset( $_elements[ $k0 ] ) ) {
						if ( isset( $all_elements[ $_elements[ $k0 ] ] ) ) {
							...
							$internal_name = $all_elements[ $_elements[ $k0 ] ]->name;
							...
							foreach ( $all_elements[ $_elements[ $k0 ] ]->properties as $key => $value ) {
								...
								if ( isset( $value['id'] ) ) {
									...
									if ( $_elements[ $k0 ] . '_clogic' === $_vid ) {
										...
										$value['default'] = wp_unslash( $value['default'] );
									}
									if ( $_elements[ $k0 ] . '_logicrules' === $_vid ) {
										if ( is_object( $value['default'] ) ) {
											$value['default'] = wp_json_encode( $value['default'] );
										}
										$value['default'] = wp_unslash( $value['default'] );
									...
									if ( 'variations_options' === $_vid ) {
										if ( $use_wpml && isset( $current_builder[ $_vid ] ) ) {
											...
											( 'multiple' === $all_elements[ $_elements[ $k0 ] ]->type
												|| 'multipleall' === $all_elements[ $_elements[ $k0 ] ]->type
												|| 'multiplesingle' === $all_elements[ $_elements[ $k0 ] ]->type
												|| 'singlemultiple' === $all_elements[ $_elements[ $k0 ] ]->type )
									) {
										...
										if ( is_null( $t[ $_vid ] ) ) {
											...
											$_extra_options = THEMECOMPLETE_EPO_BUILDER()->extra_multiple_options;

											$_extra_base = [];
											$_extra      = [];
											$_extra_keys = [];
											foreach ( $_extra_options as $__key => $__name ) {
												if ( ( is_array( $__name['type'] ) && in_array( $_elements[ $k0 ], $__name['type'], true ) ) || $_elements[ $k0 ] === $__name['type'] ) {
											...
											if ( ! is_null( $_titles_base ) && ! is_null( $_values_base ) && ! is_null( $_prices_base ) ) {
												...
												foreach ( $_titles_base as $option_key => $option_value ) {
													...
													if ( $use_original_builder ) {
														...
														$cbvalues = [];
														foreach ( $properties as $property => $property_value ) {
															if ( isset( $property_value['current'][ $_option_key ] ) && isset( $property_value['base'][ $_option_key ] ) ) {
								...
								if ( isset( $value['tags']['id'] ) && isset( $value['tags']['name'] ) ) {
									$_name = str_replace( [ '[', ']' ], '', (string) $value['tags']['name'] );
							...
							$elements_html_array[ $k0 ] = [
								...
								'fields'        => $fields,
								'label'         => $all_elements[ $_elements[ $k0 ] ]->name,
								'desc'          => $element_desc,
								'icon'          => $all_elements[ $_elements[ $k0 ] ]->icon,
	...
	public function settings_term_variations_image_helper( $value = '' ) {
		...
		echo '<span data-tm-tooltip-html="' . esc_attr( esc_html__( 'Choose the image to use in place of the radio button.', 'woocommerce-tm-extra-product-options' ) ) . '" class="tc-upload-button tm-tooltip"><i class="tcfa tcfa-upload"></i></span>';
		echo '<span class="tm_upload_image"><img class="tm_upload_image_img" alt="" src="' . esc_attr( $value ) . '"><button data-tm-tooltip-html="' . esc_attr( esc_html__( 'Remove the image.', 'woocommerce-tm-extra-product-options' ) ) . '" class="tc-button small builder-image-delete tm-tooltip" type="button"><i class="tcfa tcfa-times"></i></button></span>';
	}
	...
	public function settings_term_variations_imagep_helper( $value = '' ) {
		...
		echo '<span data-tm-tooltip-html="' . esc_attr( esc_html__( 'Choose the image to replace the product image with.', 'woocommerce-tm-extra-product-options' ) ) . '" class="tc-upload-button tc-upload-buttonp tm-tooltip"><i class="tcfa tcfa-upload"></i></span>';
		echo '<span class="tm_upload_image tm_upload_imagep"><img class="tm_upload_image_img" alt="" src="' . esc_attr( $value ) . '"><button data-tm-tooltip-html="' . esc_attr( esc_html__( 'Remove the image.', 'woocommerce-tm-extra-product-options' ) ) . '" class="tc-button small builder-image-delete tm-tooltip" type="button"><i class="tcfa tcfa-times"></i></button></span>';
	}
	...
	public function builder_sub_variations_options( $args = [] ) {
		...
		/**
		 * The data for the variations_options key in the builder
		 *
		 * @var mixed $meta
		 */
		$meta = $args['meta'];
		 ...
		 * The product id
		 *
		 * @var mixed $product_id
		 */
		$product_id = $args['product_id'];
		/**
		 * If the $js_object should be returned.
		 *
		 * @var bool $return_js
		 */
		...
		$settings_attribute = [
			...
			[
				...
				'options'          => [
					...
					[
						'text'  => esc_html__( 'Show', 'woocommerce-tm-extra-product-options' ),
						'value' => 'bottom',
					],
					[
						'text'  => esc_html__( 'Tooltip', 'woocommerce-tm-extra-product-options' ),
		...
		$settings_term = [
			...
			[
				...
				'html_before_field' => '<span class="tc-upload-image-wrap">',
				'html_after_field'  => '</span>',
				'rightclass'        => 'tc-upload-image-container',
			],
			[
		...
		foreach ( $attributes as $name => $options ) {
			...
			if ( empty( $return_js ) ) {
				echo '<div class="tma-handle-wrap tm-attribute">'
					. '<div class="tma-handle"><div class="tma-attribute-label">'
			...
			foreach ( $settings_attribute as $setting ) {
				...
				if ( ! empty( $meta )
					...
					if ( 'variations_show_name' === $setting['id'] && 'inside' === $setting['default'] ) {
				...
				if ( taxonomy_exists( $taxonomy_name ) ) {
					...
					if ( ! empty( $terms ) && is_array( $terms ) ) {
						...
						foreach ( $terms as $term ) {
							...
							$has_term = has_term( intval( $term->term_id ), $taxonomy_name, $product_id );
							if ( ! $has_term ) {
								continue;
							}
							...
							foreach ( $settings_term as $setting ) {
								...
								$setting['tags']['name'] = str_replace( '%term_id%', ( $term_id ), $setting['tags']['name'] );
					...
					foreach ( $options as $option ) {
						...
						foreach ( $settings_term as $setting ) {
							...
							$setting['tags']['name'] = str_replace( '%id%', $setting['id'], $setting['tags']['name'] );
							$setting['tags']['name'] = str_replace( '%attribute_id%', sanitize_title( THEMECOMPLETE_EPO_HELPER()->sanitize_key( $name ) ), $setting['tags']['name'] );
							$setting['tags']['name'] = str_replace( '%term_id%', ( $option ), $setting['tags']['name'] );
	...
	public function get_builder_sub_options_upload_helper( $name = '' ) {
		if ( 'multiple_radiobuttons_options' === $name || 'multiple_checkboxes_options' === $name ) {
			if ( 'multiple_radiobuttons_options' === $name ) {
				...
				echo '<span data-tm-tooltip-html="' . esc_attr( esc_html__( 'Choose the image to use in place of the checkbox.', 'woocommerce-tm-extra-product-options' ) ) . '" class="tc-upload-button tm-tooltip"><i class="tcfa tcfa-upload"></i></span>';
			}
		}
	}
	...
	public function builder_sub_options_image_helper( $name = '', $image = false ) {
		...
		if ( false !== $image ) {
			...
			echo '<span class="tm_upload_image"><img class="tm_upload_image_img" alt="" src="' . esc_attr( $image ) . '" /><button data-tm-tooltip-html="' . esc_attr( esc_html__( 'Remove the image.', 'woocommerce-tm-extra-product-options' ) ) . '" class="tc-button small builder-image-delete tm-tooltip" type="button"><i class="tcfa tcfa-times"></i></button></span>';
		}
	}
	...
	public function get_builder_sub_options_uploadc_helper( $name = '' ) {
		if ( 'multiple_radiobuttons_options' === $name || 'multiple_checkboxes_options' === $name ) {
			if ( 'multiple_radiobuttons_options' === $name ) {
				echo '<span data-tm-tooltip-html="' . esc_attr( esc_html__( 'Choose the image to use in place of the radio button when it is checked.', 'woocommerce-tm-extra-product-options' ) ) . '" class="tc-upload-button tm-tooltip"><i class="tcfa tcfa-upload"></i></span>';
			} elseif ( 'multiple_checkboxes_options' === $name ) {
				echo '<span data-tm-tooltip-html="' . esc_attr( esc_html__( 'Choose the image to use in place of the checkbox when it is checked.', 'woocommerce-tm-extra-product-options' ) ) . '" class="tc-upload-button tm-tooltip"><i class="tcfa tcfa-upload"></i></span>';
			}
		}
	}
	...
	public function builder_sub_options_imagec_helper( $name = '', $image = false ) {
		...
		if ( false !== $image ) {
			...
			echo '<span class="tm_upload_image tm_upload_imagec"><img class="tm_upload_image_img" alt="" src="' . esc_attr( $image ) . '" /><button data-tm-tooltip-html="' . esc_attr( esc_html__( 'Remove the image.', 'woocommerce-tm-extra-product-options' ) ) . '" class="tc-button small builder-image-delete tm-tooltip" type="button"><i class="tcfa tcfa-times"></i></button></span>';
		}
	}
	...
	public function builder_sub_options_imagep_helper( $name = '', $image = false ) {
		...
		if ( false !== $image ) {
			...
			echo '<span class="tm_upload_image tm_upload_imagep"><img class="tm_upload_image_img" alt="" src="' . esc_attr( $image ) . '" /><button data-tm-tooltip-html="' . esc_attr( esc_html__( 'Remove the image.', 'woocommerce-tm-extra-product-options' ) ) . '" class="tc-button small builder-image-delete tm-tooltip" type="button"><i class="tcfa tcfa-times"></i></button></span>';
		}
	}
	...
	public function get_builder_sub_options_uploadl_helper( $name = '' ) {
		if ( 'multiple_radiobuttons_options' === $name || 'multiple_checkboxes_options' === $name ) {
			echo '<span data-tm-tooltip-html="' . esc_attr( esc_html__( 'Choose the image for the lightbox.', 'woocommerce-tm-extra-product-options' ) ) . '" class="tc-upload-button tc-upload-buttonl tm-tooltip"><i class="tcfa tcfa-upload"></i></span>';
		} elseif ( 'multiple_selectbox_options' === $name ) {
			echo '<span data-tm-tooltip-html="' . esc_attr( esc_html__( 'Choose the image for the lightbox.', 'woocommerce-tm-extra-product-options' ) ) . '" class="tc-upload-button tc-upload-buttonl tm-tooltip"><i class="tcfa tcfa-upload"></i></span>';
		}
	}
	...
	public function builder_sub_options_imagel_helper( $name = '', $image = false ) {
		...
		if ( false !== $image ) {
			...
			echo '<span class="tm_upload_image tm_upload_imagel"><img class="tm_upload_image_img" alt="" src="' . esc_attr( $image ) . '" /><button data-tm-tooltip-html="' . esc_attr( esc_html__( 'Remove the image.', 'woocommerce-tm-extra-product-options' ) ) . '" class="tc-button small builder-image-delete tm-tooltip" type="button"><i class="tcfa tcfa-times"></i></button></span>';
		}
	}
	 ...
	 * @access public
	 */
	public function remove_for_js( $setting = [] ) {
		unset( $setting['extra'] );

		if ( isset( $setting['type'] ) && 'checkbox' === $setting['type'] ) {
			$setting['checked'] = '' !== checked( ( $setting['default'] === $setting['tags']['value'] ), true, false );
		}
	...
	public function builder_sub_options( $args = [] ) {
		 ...
		 * @var mixed $type
		 */
		$type = str_replace( [ 'multiple_', '_options' ], '', $name );
		/**
		 * The choice counter
		 *
		 * @var mixed $counter
		...
		if ( ! $options ) {
			...
			foreach ( $_extra_options as $__key => $__name ) {
				if ( ( is_array( $__name['type'] ) && in_array( $type, $__name['type'], true ) ) || $__name['type'] === $type ) {
					$options[ $__name['name'] ] = [ '' ];
				}
			}
		}
		...
		if ( 'multiple_selectbox_options' === $name || 'multiple_radiobuttons_options' === $name || 'multiple_checkboxes_options' === $name ) {
			$class = ' withupload';
		}
		...
		$o['enabled']['default'] = '1';
		$o['enabled']['tags']    = [
			'class' => 'c tc-option-enabled ' . $name,
			'value' => '1',
		];
		...
		if ( empty( $return_js ) ) {
			...
			echo '<div class="tc-cell tc-col-auto"><button type="button" class="tc tc-button builder_panel_delete_all">'
				. esc_html__( 'Delete all options', 'woocommerce-tm-extra-product-options' ) . '</button></div>'
				. '</div>';
		}
		...
		if ( $total_entries > $per_page ) {
			...
			if ( empty( $return_js ) ) {
				echo '<div data-perpage="' . esc_attr( (string) $per_page ) . '" data-totalpages="' . esc_attr( (string) $pages ) . '" class="tcpagination tc-clearfix"></div>';
			}
		} elseif ( empty( $return_js ) ) {
			echo '<div data-perpage="' . esc_attr( (string) $per_page ) . '" data-totalpages="0" class="tcpagination tc-clearfix"></div>';
		}
		if ( empty( $return_js ) ) {
		...
		foreach ( $options['title'] as $ar => $el ) {
			...
			if ( ! is_array( $options['sale_price'] ) ) {
				$options['sale_price'] = [];
			}
			if ( ! is_array( $options['image'] ) ) {
				$options['image'] = [];
			}
			if ( ! is_array( $options['imagec'] ) ) {
				$options['imagec'] = [];
			}
			if ( ! is_array( $options['imagep'] ) ) {
				$options['imagep'] = [];
			}
			if ( ! is_array( $options['imagel'] ) ) {
				$options['imagel'] = [];
			}
			if ( ! is_array( $options['price_type'] ) ) {
				$options['price_type'] = [];
			}
			...
			if ( ! is_array( $options['fee'] ) ) {
				$options['fee'] = [];
			}
			if ( ! is_array( $options['dnmpbq'] ) ) {
				$options['dnmpbq'] = [];
			}
			...
			if ( ! isset( $options['dnmpbq'][ $ar ] ) ) {
				$options['dnmpbq'][ $ar ] = $original_o['dnmpbq']['default'];
			}
			foreach ( $_extra_options as $__key => $__name ) {
				if ( ( is_array( $__name['type'] ) && in_array( $type, $__name['type'], true ) ) || $__name['type'] === $type ) {
					$_extra_name = $__name['name'];
			...
			if ( empty( $return_js ) ) {
				...
				echo '<div class="tc-cell tc-col-12 tc-cell-control">';
				...
				echo '<span class="tm-cell-enabled">';
				echo '<span class="tm-choice-cl enabled">' . esc_html__( 'Enabled', 'woocommerce-tm-extra-product-options' ) . '</span>';
				echo '<span class="tm-choice-cl disabled">' . esc_html__( 'Disabled', 'woocommerce-tm-extra-product-options' ) . '</span>';
			...
			if ( empty( $return_js ) ) {
				echo '<span class="tm-choice-price-wrap fullwidth">';
				echo '<span class="tm-choice-cl">' . esc_html__( 'Choice label', 'woocommerce-tm-extra-product-options' ) . '</span>';
				...
				echo '</div>';
				echo "<div class='tc-cell tc-col-2 tc-cell-price-type'>";
				echo '<span class="tm-choice-price-wrap fullwidth">';
				echo '<span class="tm-choice-pt">' . esc_html__( 'Price Type', 'woocommerce-tm-extra-product-options' ) . '</span>';
				THEMECOMPLETE_EPO_HTML()->create_field( $o['price_type'], 1 );
				echo '</span>';
				echo '</div>';
				echo "<div class='tc-cell tc-col-5 tc-cell-price'>";
			} else {
			...
			if ( ! empty( $additional_currencies ) && is_array( $additional_currencies ) ) {
				...
				$o['price']['html_before_field']      = '<span class="tm-choice-price-wrap"><span class="tm-choice-currency">' . THEMECOMPLETE_EPO_HELPER()->wc_base_currency() . '</span><span class="tm-choice-regular">' . esc_html__( 'Price', 'woocommerce-tm-extra-product-options' ) . '</span>';
				$o['sale_price']['html_before_field'] = '<span class="tm-choice-price-wrap"><span class="tm-choice-currency">' . THEMECOMPLETE_EPO_HELPER()->wc_base_currency() . '</span><span class="tm-choice-sale">' . esc_html__( 'Sale Price', 'woocommerce-tm-extra-product-options' ) . '</span>';
				$o['price']['html_after_field']       = '</span>';
				$o['sale_price']['html_after_field']  = '</span>';
				if ( empty( $return_js ) ) {
				...
				foreach ( $additional_currencies as $ckey => $currency ) {
					...
					$copy_value['html_before_field'] = '<span class="tm-choice-price-wrap"><span class="tm-choice-currency">' . $currency . '</span><span class="tm-choice-regular">' . esc_html__( 'Price', 'woocommerce-tm-extra-product-options' ) . '</span>';
					$copy_value['html_after_field']  = '</span>';
					...
					$copy_value['html_before_field'] = '<span class="tm-choice-price-wrap"><span class="tm-choice-currency">' . $currency . '</span><span class="tm-choice-sale">' . esc_html__( 'Sale Price', 'woocommerce-tm-extra-product-options' ) . '</span>';
					$copy_value['html_after_field']  = '</span>';
				...
				$o['price']['html_before_field']      = '<span class="tm-choice-price-wrap"><span class="tm-choice-regular">' . esc_html__( 'Price', 'woocommerce-tm-extra-product-options' ) . '</span>';
				$o['sale_price']['html_before_field'] = '<span class="tm-choice-price-wrap"><span class="tm-choice-sale">' . esc_html__( 'Sale Price', 'woocommerce-tm-extra-product-options' ) . '</span>';
				$o['price']['html_after_field']       = '</span>';
				$o['sale_price']['html_after_field']  = '</span>';
				if ( empty( $return_js ) ) {
					THEMECOMPLETE_EPO_HTML()->create_field( $o['price'], 1 );
			...
			if ( empty( $return_js ) ) {
				echo '</div>';
				echo '<div class="tc-cell tc-col-12 tc-cell-images tc-upload-image-container">';
				echo '<span class="tc-upload-image-wrap swatch">';
				echo '<span class="tm-choice-info">' . esc_html__( 'Swatch', 'woocommerce-tm-extra-product-options' ) . '</span>';
				THEMECOMPLETE_EPO_HTML()->create_field( $o['image'], 1 );
				echo '</span>';
				echo '<span class="tc-upload-image-wrap swatch-hover">';
				echo '<span class="tm-choice-info">' . esc_html__( 'Swatch (Hover)', 'woocommerce-tm-extra-product-options' ) . '</span>';
				THEMECOMPLETE_EPO_HTML()->create_field( $o['imagec'], 1 );
				echo '</span>';
				echo '<span class="tc-upload-image-wrap product-image">';
				echo '<span class="tm-choice-info">' . esc_html__( 'Product Image', 'woocommerce-tm-extra-product-options' ) . '</span>';
				THEMECOMPLETE_EPO_HTML()->create_field( $o['imagep'], 1 );
				echo '</span>';
				echo '<span class="tc-upload-image-wrap lightbox">';
				echo '<span class="tm-choice-info">' . esc_html__( 'Lightbox', 'woocommerce-tm-extra-product-options' ) . '</span>';
				THEMECOMPLETE_EPO_HTML()->create_field( $o['imagel'], 1 );
				echo '</span>';
				if ( 'multiple_selectbox_options' !== $name ) {
					echo '<span class="tc-upload-image-wrap color">';
					echo '<span class="tm-choice-info">' . esc_html__( 'Color Swatch', 'woocommerce-tm-extra-product-options' ) . '</span>';
				...
				if ( 'multiple_checkboxes_options' === $name ) {
					echo '<div class="' . esc_attr( apply_filters( 'wc_epo_builder_element_multiple_checkboxes_options_class', 'tc-cell tc-col-12 tc-cell-fee', $o ) ) . '"><span class="tm-choice-price-wrap fullwidth"><span class="tm-choice-cl">' . esc_html__( 'Set to Fee', 'woocommerce-tm-extra-product-options' ) . '</span>';
					THEMECOMPLETE_EPO_HTML()->create_field( $o['fee'], 1 );
					echo '</span></div>';
					echo '<div class="tc-cell tc-col-12 tc-cell-dnmpbq"><span class="tm-choice-price-wrap fullwidth"><span class="tm-choice-cl">' . esc_html__( 'Do Not Multiply Price by Quantity', 'woocommerce-tm-extra-product-options' ) . '</span>';
					THEMECOMPLETE_EPO_HTML()->create_field( $o['dnmpbq'], 1 );
					echo '</span></div>';
					do_action( 'wc_epo_builder_element_multiple_checkboxes_options', $o );
				}
				if ( 'multiple_selectboxmultiple_options' !== $name ) {
					echo '<div class="tc-cell tc-col-12 tc-cell-description"><span class="tm-choice-price-wrap fullwidth"><span class="tm-choice-cl">' . esc_html__( 'Description', 'woocommerce-tm-extra-product-options' ) . '</span>';
				...
				if ( 'multiple_checkboxes_options' === $name ) {
					...
					$js_object[ $d_counter ][] = [
						'id'      => $name . '_dnmpbq',
						'default' => (string) $o['dnmpbq']['default'],
						'checked' => (string) $o['dnmpbq']['default'] === (string) $o['dnmpbq']['tags']['value'],
						'type'    => 'checkbox',
						'tags'    => [
							'name'  => 'tm_meta[tmfbuilder][' . $name . '_dnmpbq][' . ( is_null( $counter ) ? 0 : $counter ) . '][]',
							'value' => $o['dnmpbq']['tags']['value'],
						],
					];
			...
			foreach ( $_extra_options as $__key => $__name ) {
				if ( ( is_array( $__name['type'] ) && in_array( $type, $__name['type'], true ) ) || $__name['type'] === $type ) {
					...
					if ( empty( $return_js ) ) {
						...
						echo '<span class="tm-choice-price-wrap fullwidth"><span class="tm-choice-cl">' . esc_attr( $__name['label'] ) . '</span>';
			...
			if ( empty( $return_js ) ) {
				echo '<div class="tc-cell tc-col-12 tc-cell-url"><span class="tm-choice-price-wrap fullwidth"><span class="tm-choice-cl">' . esc_html__( 'URL', 'woocommerce-tm-extra-product-options' ) . '</span>';
