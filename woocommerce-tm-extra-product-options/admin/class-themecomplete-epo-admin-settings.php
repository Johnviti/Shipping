...
if ( class_exists( 'WC_Settings_Page' ) ) {
	...
	class THEMECOMPLETE_EPO_ADMIN_SETTINGS extends WC_Settings_Page {
		...
		public function __construct() {
			...
			$this->settings_array   = THEMECOMPLETE_EPO_SETTINGS()->plugin_settings_data();
		...
		public function script_templates() {
			if ( isset( $_GET['page'] ) && ( 'tcepo-settings' === $_GET['page'] || ( 'wc-settings' === $_GET['page'] && isset( $_GET['tab'] ) && 'tm_extra_product_options' === $_GET['tab'] ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		...
		public function tm_tabs_header_setting() {
			...
			echo '<h2>' . esc_html__( 'EPO Control Panel', 'woocommerce-tm-extra-product-options' ) . ' <span>' . esc_html( THEMECOMPLETE_EPO_VERSION ) . '</span></h2>';
			echo '</div>';
			...
			echo '</div>';
			echo '<div class="header tm-flexrow"><h3 class="tm-flexcol">' . esc_html__( 'Extra Product Options Settings', 'woocommerce-tm-extra-product-options' ) . '</h3>';
			echo '<div class="tm-flexcol">';
			echo '<button type="submit" class="tc tc-button tc-reset-button" type="submit">' . esc_html__( 'Reset settings', 'woocommerce-tm-extra-product-options' ) . '</button>';
			echo '<button type="submit" class="tc tc-button tc-save-button" type="submit">' . esc_html__( 'Save changes', 'woocommerce-tm-extra-product-options' ) . '</button>';
			echo '</div>';
			echo '</div>';
		...
		public function tm_settings_hook_all_end() {
			...
			echo '<div class="tm-footer tm-flexrow">';
			echo '<div class="tm-flexcol">';
			echo '<button type="submit" class="tc tc-button tc-reset-button" type="submit">' . esc_html__( 'Reset settings', 'woocommerce-tm-extra-product-options' ) . '</button>';
			echo '<button type="submit" class="tc tc-button tc-save-button" type="submit">' . esc_html__( 'Save changes', 'woocommerce-tm-extra-product-options' ) . '</button>';
			echo '</div>';
