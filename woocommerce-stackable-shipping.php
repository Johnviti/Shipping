...
if (!class_exists('WC_Stackable_Shipping')) {
    class WC_Stackable_Shipping {
        ...
        public function init() {
            ...
            // Adicionar campos personalizados na edição de produtos
            add_action('woocommerce_product_options_dimensions', array($this, 'add_stackable_product_fields'));
            add_action('woocommerce_process_product_meta', array($this, 'save_stackable_product_fields'));

            // Adicionar página de administração para configuração de grupos
        ...
        /**
         * Adicionar campos aos produtos para configuração de empilhamento
         */
        public function add_stackable_product_fields() {
            ...
            // Campo para ativar empilhamento
            woocommerce_wp_checkbox(array(
                'id' => '_is_stackable',
                'label' => __('Produto Empilhável', 'woocommerce-stackable-shipping'),
                'description' => __('Marque se este produto pode ser empilhado com outros para cálculo de frete', 'woocommerce-stackable-shipping')
            ));
            
            // Número máximo de empilhamento do mesmo produto
            woocommerce_wp_text_input(array(
                'id' => '_max_stack_same',
                'label' => __('Empilhamento Máximo (mesmo produto)', 'woocommerce-stackable-shipping'),
                'description' => __('Quantidade máxima deste produto que pode ser empilhada', 'woocommerce-stackable-shipping'),
                'type' => 'number',
            ...
            // Dimensões adicionais ao empilhar (altura)
            woocommerce_wp_text_input(array(
                'id' => '_stack_height_increment',
                'label' => __('Incremento de Altura ao Empilhar (cm)', 'woocommerce-stackable-shipping'),
                'description' => __('Altura adicional ao empilhar cada unidade após a primeira', 'woocommerce-stackable-shipping'),
                'type' => 'number',
                'custom_attributes' => array(
                    'step' => '0.01',
                    'min' => '0'
        ...
        public function save_stackable_product_fields($product_id) {
            // Salvar campo de produto empilhável
            $is_stackable = isset($_POST['_is_stackable']) ? 'yes' : 'no';
            update_post_meta($product_id, '_is_stackable', $is_stackable);
            
            // Salvar quantidade máxima de empilhamento
            if (isset($_POST['_max_stack_same'])) {
                update_post_meta($product_id, '_max_stack_same', absint($_POST['_max_stack_same']));
            }
            
            // Salvar incremento de altura
            if (isset($_POST['_stack_height_increment'])) {
                update_post_meta($product_id, '_stack_height_increment', wc_format_decimal($_POST['_stack_height_increment']));
            }
        }
        ...
        /**
         * Agrupa produtos empilháveis do carrinho
         */
        private function group_stackable_products($cart_contents) {
            ...
            $stackable_groups = array();
            foreach ($stackable_products as $cart_item_key => $cart_item) {
                $product_id = $cart_item['product_id'];
                
                if (!isset($stackable_groups[$product_id])) {
                    $stackable_groups[$product_id] = array();
                }
                
                $stackable_groups[$product_id][$cart_item_key] = $cart_item;
            }
            
            // Para cada grupo, verificamos se está dentro do limite de empilhamento
            $final_groups = array();
            foreach ($stackable_groups as $product_id => $items) {
